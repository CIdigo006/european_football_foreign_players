...
def get_champions_league_clubs():
    ...
    querystring = {"id":input(),"domain":api_domain}
    headers = api_headers
    response = requests.get(url, headers=headers, params=querystring)
    champions_league_clubs_json = response.json()["clubs"]
    breakpoint()
    return champions_league_clubs_json
...
def get_leagues_from_clubs(clubs_dict):

    clubs_json = clubs_dict

    # loop through the list of dictionary of clubs. With each loop hit the api to get the domestic league each team in the
    # the list belongs to and add them to the 'clubs_json' list of dictionaries as "league": "league identifier" then from there add 
    ...
    # breakpoint()
    return (clubs_json, clubs_leagues_cleaned)